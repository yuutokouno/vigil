...
class BugBashEvent(Base):
    __tablename__ = "bug_bash_events"
...
class BugBashParticipation(Base):
    __tablename__ = "bug_bash_participations"

    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    event_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("bug_bash_events.id", ondelete="CASCADE"), nullable=False
    )
    user_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("users.id", ondelete="CASCADE"), nullable=False
    )
    ...
    __table_args__ = (
        UniqueConstraint("event_id", "user_id", name="uq_bug_bash_participation"),
    )


class BugBashSubmission(Base):
    __tablename__ = "bug_bash_submissions"
