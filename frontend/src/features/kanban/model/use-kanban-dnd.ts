...
/**
 * Resolves over.id to a status slug.
 * over.id can be a column slug or a card id (bug UUID).
 */
...
export function useKanbanDnd(
  ...
  const validSlugsRef = useRef(validSlugs ?? []);
  validSlugsRef.current = validSlugs ?? [];
  const originalBugRef = useRef<{ id: string; title: string; status: string } | null>(null);
  ...
  const handleDragOver = ({ active, over }: DragOverEvent) => {
    ...
    const newStatus = resolveStatus(String(over.id), bugsRef.current, validSlugsRef.current);
    if (!newStatus) return;
